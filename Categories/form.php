    ...
    <div class="row justify-content-center mx-3 mt-4">
        ...
        <form method="POST" action="<?php echo $isEdit ? 'edit.php' : 'add.php'; ?>" class="row g-3">
            ...
            <div class="col-12">
                ...
                <input type="text" class="form-control" id="name" name="category[name]" value="<?php echo htmlspecialchars($categoryModel->value('name') ?? ''); ?>" placeholder="e.g. Electronics" required>
            </div>

            <div class="col-12">
                <label for="description" class="form-label fw-semibold text-secondary small text-uppercase">Description</label>
                <textarea class="form-control" id="description" name="category[description]" rows="4" placeholder="Describe this category..."><?php echo htmlspecialchars($categoryModel->value('description') ?? ''); ?></textarea>
            </div>
