    ...
    <div class="row justify-content-center mx-3 mt-4">
        ...
        <form method="POST" action="<?php echo $isEdit ? 'edit.php' : 'add.php'; ?>" class="row g-3">
            ...
            <div class="col-12">
                ...
                <input type="text" class="form-control" id="name" name="customer_group[group_name]" value="<?php echo htmlspecialchars($customerGroupModel->value('group_name') ?? ''); ?>" placeholder="e.g. VIP Customers" required>
            </div>

            <div class="col-12">
                <label for="description" class="form-label fw-semibold text-secondary small text-uppercase">Description</label>
                <textarea class="form-control" id="description" name="customer_group[description]" rows="4" placeholder="Briefly explain the purpose of this group..."><?php echo htmlspecialchars($customerGroupModel->value('description') ?? ''); ?></textarea>
            </div>
